...
export default function Home() {
  ...
  const getPic = async () => {
    ...
    return fetch(`https://api.waifu.pics/sfw/${category[categoryIndex]}`)
      .then(response => response.json())
      .then((data) => { setLoad(false); return data.url })
...
<div className={dark ? "dark" : null}>
    <div className="h-screen w-screen dark:bg-black bg-gray-200 relative flex justify-center transition-colors">
      ...
      <div className="top-1/2 absolute flex justify-center flex-col">
        ...
        <button onClick={() => { setLoad(true); getPic().then(response => window.open(response)) }} className={"text-white  text-zl rounded-md py-1 mt-2  bg-pink-400 hover:bg-pink-500 inline-flex justify-center items-center"}>{load ?
